...
describe('17 - Renderize `<SearchBar />` dentro de `<MovieLibrary />`', () => {
  xit('Renderize o componente `<SearchBar />`', () => {
    const { getAllByTestId } = render(<MovieLibrary movies={ movies } />);
    const searchBar = getAllByTestId('search-bar-form');
    expect(searchBar).toHaveLength(1);
  });

  xit('Altere o estado da `<SearchBar />` quando algo for digitado nela', () => {
    const { getByTestId } = render(<MovieLibrary movies={ movies } />);
    const searchText = getByTestId(textTestId);
  ...
  xit('Disponibilize a opção de filtrar por favoritos', () => {
    const { getByTestId } = render(<MovieLibrary movies={ movies } />);
    const bookmarkedOnly = getByTestId(checkboxTestId);
  ...
  xit('Disponibilize a opção de filtrar por categorias', () => {
    const { getByTestId } = render(<MovieLibrary movies={ movies } />);
    const selectInput = getByTestId(selectTestId);
    expect(selectInput).toHaveValue('');
...
describe('18 - Renderize `<MovieList />` dentro de `<MovieLibrary />`', () => {
  ...
  xit('Filtre os filmes por título de acordo com o que for digitado na barra de busca', () => {
  ...
  xit('Filtre os filmes por subtítulo de acordo com o que for digitado na barra de busca', () => {
    const { getByTestId, getAllByTestId } = render(<MovieLibrary movies={ movies } />);
    const textInput = getByTestId(textTestId);
  ...
  xit('Filtre os filmes por sinopse de acordo com o que for digitado na barra de busca', () => {
    const { getByTestId, getAllByTestId } = render(<MovieLibrary movies={ movies } />);
    const textInput = getByTestId(textTestId);
  ...
  xit('Renderize a lista de filmes sem filtragens se a barra de buscar estiver vazia', () => {
  ...
  xit('Filtre os filmes por favoritos quando a `checkbox` relacionada for selecionada', () => {
    const { getByTestId, getAllByTestId } = render(<MovieLibrary movies={ movies } />);
    const bookmarkedOnly = getByTestId(checkboxTestId);
  ...
  xit('Filtre os filmes por categoria quando a `checkbox` relacionada for selecionada', () => {
    const { getByTestId, getAllByTestId } = render(<MovieLibrary movies={ movies } />);
...
describe('19 - Renderize `<AddMovie />` dentro de `<MovieLibrary />`', () => {
  xit('Renderize o componente `<AddMovie />`', () => {
    const { getAllByTestId } = render(<MovieLibrary movies={ movies } />);
    const addMovieForm = getAllByTestId('add-movie-form');
    expect(addMovieForm).toHaveLength(1);
  });

  xit('Adicione, após preenchimento do formulário e clique no botão de enviar, o novo filme à lista de filmes', () => {
    const { getByTestId, getAllByTestId } = render(<MovieLibrary movies={ movies } />);
